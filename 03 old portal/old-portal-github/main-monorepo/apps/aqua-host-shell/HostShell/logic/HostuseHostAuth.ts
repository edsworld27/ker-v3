...
import type { HostUser, BridgeSession } from '@HostShell/bridge/types';
...
export function useHostAuth(persisted: any, deps: AuthLogicDeps) {
  ...
  const [loginHostPortalType, setLoginHostPortalType] = useState<'agency' | 'client'>(persisted?.loginHostPortalType ?? 'agency');
  ...
  const handleLogin = async (email: string, pass: string) => {
    const response = await BridgeAPI.authenticate(email, loginHostPortalType);
  ...
  return {
    ...
    toggleAuthMode,
    code, setCode,
    loginHostPortalType, setLoginHostPortalType,
