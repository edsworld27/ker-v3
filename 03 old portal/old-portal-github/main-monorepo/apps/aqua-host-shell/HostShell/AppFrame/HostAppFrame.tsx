...
import { HostDemoShell } from '../components/HostDemoShell';
import { HostSidebarContent } from '../Sidebar/HostSidebarContent';
import { DynamicViewRenderer } from '../Renderer/HostDynamicViewRenderer';
import { ModalManager } from '@HostShell/components/HostModalManager';
...
import { WelcomeScreen } from '../components/Auth/HostWelcomeScreen';
import { LoginView } from '../components/Auth/HostLoginView';
import { SecurityCheckView } from '../components/Auth/HostSecurityCheckView';
    ...
    if (this.state.error) {
      ...
      return (
        <div className="flex flex-col items-center justify-center p-16 text-center">
          <p className="text-xl font-bold text-[var(--host-widget-error)] mb-2">Something went wrong in this view</p>
          <p className="text-sm text-[var(--host-text-color-muted)] mb-4 font-mono">{err.message}</p>
...
interface HostFrameProps {
  portalMode?: 'demo' | 'user'; // kept for compat; context value takes precedence when dev bar is active
}

/**
 * HostFrame — The Structural Host
 *
 ...
 */
export const HostFrame: React.FC<HostFrameProps> = ({ portalMode: portalModeProp }) => {
  // Track HostRegistration updates so ViewErrorBoundary auto-resets on template load
  const [registryTick, setRegistryTick] = useState(0);
  useEffect(() => HostRegistration.subscribe(() => setRegistryTick(t => t + 1)), []);
  ...
  // Context portalMode wins (dev bar can toggle it); fall back to prop
  const portalMode = contextPortalMode ?? portalModeProp ?? 'user';
  ...
  return (
    <div className={`relative flex min-h-screen overflow-hidden transition-colors duration-1000 ${step === 'portal' ? 'bg-black' : 'bg-[#0f172a]'}`}>
      <AnimatePresence mode="wait">
        ...
        {step === 'portal' && portalMode === 'demo' && (
          <motion.div key="demo-portal" initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="w-full h-screen">
            <HostDemoShell />
          </motion.div>
        )}

        {step === 'portal' && portalMode === 'user' && mfaVerified && (
          <motion.div
            key="portal"
            initial={{ opacity: 0 }}
            animate={{ opacity: 1 }}
      ...
      {/* Dev Mode Toolbar */}
      {appMode === 'dev' && (
        <div className="fixed bottom-4 left-1/2 -translate-x-1/2 z-[9999] flex items-center gap-3 px-4 py-2.5 rounded-2xl bg-black/80 border border-white/10 backdrop-blur-xl shadow-2xl">
          <div className="flex items-center gap-1.5 text-[10px] font-mono font-bold uppercase tracking-widest text-white/40">
          ...
          <div className="flex items-center gap-1 p-0.5 bg-white/5 rounded-xl border border-white/5">
            <button
              onClick={() => setHostPortalMode('user')}
            ...
            <button
              onClick={() => setHostPortalMode('demo')}
              className={`px-3 py-1 rounded-lg text-[10px] font-bold uppercase tracking-widest transition-all ${
                portalMode === 'demo'
