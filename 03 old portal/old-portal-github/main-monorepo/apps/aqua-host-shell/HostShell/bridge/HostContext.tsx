...
import { 
  HostUser, Step, HostPortalView, HostPortalTier, Agency, Client, LogEntry, HostNotification, ClientStage
} from '@HostShell/bridge/types';

export interface HostContextType {
  // ── Core Data ─────────────────────────────────────────────────────────────
  currentUser: HostUser | undefined;
  users: HostUser[];
  ...
  portalTier: HostPortalTier;
  setHostPortalTier: React.Dispatch<React.SetStateAction<HostPortalTier>>;
  ...
  setHostPortalView: React.Dispatch<React.SetStateAction<HostPortalView | string>>;
  ...
  activeHostPortalTheme: string;
  setActiveHostPortalTheme: React.Dispatch<React.SetStateAction<string>>;
  ...
  loginHostPortalType: 'agency' | 'client';
  setLoginHostPortalType: React.Dispatch<React.SetStateAction<'agency' | 'client'>>;
  ...
  userProfile: HostUser | undefined;
  
  // ── Deployment Modes ─────────────────────────────────────────────────────
  designMode: boolean;
  setDesignMode: React.Dispatch<React.SetStateAction<boolean>>;
  portalMode: 'demo' | 'user';
  setHostPortalMode: React.Dispatch<React.SetStateAction<'demo' | 'user'>>;
...
export const HostProvider: React.FC<{ children: ReactNode, value: HostContextType }> = ({ children, value }) => {
...
export const useHostContext = () => {
  ...
  if (context === undefined) {
    throw new Error('useHostContext must be used within an HostProvider');
  }
  return context;
};
