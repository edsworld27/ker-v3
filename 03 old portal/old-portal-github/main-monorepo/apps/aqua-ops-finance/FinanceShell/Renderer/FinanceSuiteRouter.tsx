...
export const SuiteRouter: React.FC<SuiteRouterProps> = ({ 
  ...
  const { portalView } = useFinanceContext();

  // Force re-render when FinanceRegistration updates
  const [, setTick] = useState(0);
  useEffect(() => {
    return FinanceRegistration.subscribe(() => setTick(t => t + 1));
  }, []);
  ...
  const Component = FinanceRegistration.resolve(componentIdToResolve) || SmartRegistry[componentIdToResolve];
  ...
  if (defaultViewId && componentIdToResolve !== defaultViewId) {
    const DefaultComponent = FinanceRegistration.resolve(defaultViewId) || SmartRegistry[defaultViewId];
    if (DefaultComponent) {
