...
} from '@FinanceShell/bridge/data/FinancemockData';
import { loadSeedData } from '@FinanceShell/bridge/data/Financedb';
import { initialMasterConfig } from '@FinanceShell/bridge/config/FinancemasterConfig';
import { BridgeProvisioning } from '@FinanceShell/bridge/Financeprovisioning';
import { agencyConfig as defaultAgencyConfig } from '@FinanceShell/bridge/config/FinanceagencyConfig';
import { useSyncStore } from '@FinanceShell/hooks/FinanceuseSyncStore';
import { BridgeAPI } from '@FinanceShell/bridge/Financeapi';

/**
 * useFinanceCore — The Central Data Kernel
 * 
 ...
 */
export function useFinanceCore(persisted: any) {
  ...
  /**
   * Called by useFinanceAuth immediately after a successful login,
  ...
  const addLog = (action: string, details: string, type: any = 'system', clientId?: string) => {
    const newLog: LogEntry = { 
      ...
      userId: 0, // Resolved at useFinanceLogic level 
      userName: 'System', 
      action, 
      details, 
      type 
  ...
  return {
    ...
    handleImpersonate: (clientId: string) => {
      // This will be overridden in useFinanceLogic with actual auth/shell logic, 
