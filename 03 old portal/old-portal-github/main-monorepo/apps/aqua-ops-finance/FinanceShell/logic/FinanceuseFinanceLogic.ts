...
import { useFinanceCore } from '@FinanceShell/logic/FinanceuseFinanceCore';
import { useFinanceAuth } from '@FinanceShell/logic/FinanceuseAuthLogic';
import { useFinanceShellLogic } from '@FinanceShell/logic/FinanceuseShellLogic';
...
/**
 * useFinanceLogic (The Facade Orchestrator)
 * 
...
export function useFinanceLogic() {
  ...
  const core = useFinanceCore(persisted);
  const {
  ...
  const auth = useFinanceAuth(persisted, {
    users,
    currentUserEmail,
    addLog,
    sendNotification,
  ...
  const shell = useFinanceShellLogic(persisted, { addLog });
  ...
  // ── Demo Mode Engine ──────────────────────────────────────────────────────
  const setDemoState = () => {
    import('@FinanceShell/bridge/demo/FinancedemoState').then(module => {
  ...
  // ── Missing FinanceContext members ────────────────────────────────────────────
  ...
  // Keep ref in sync so useFinanceAuth can call it
