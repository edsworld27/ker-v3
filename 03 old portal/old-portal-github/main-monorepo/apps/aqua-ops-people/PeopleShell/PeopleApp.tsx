 ...
 * 3. Provide the PeopleContext to all downstream components.
...
export default function App({ mode: initialMode = 'user', initialView }: AppProps) {
  ...
  useEffect(() => {
    ...
    if (agencyConfig?.identity) {
      ...
      root.style.setProperty('--people-secondary-color', agencyConfig.identity.secondaryColor || '#10b981');
    }
  ...
  // the PeopleFrame will internally render the Auth views based on the 'step' state.
  
  return (
    <PeopleProvider value={appContextValue as any}>
      <PeopleBridgeHub>
        <PeopleFrame portalMode={portalMode} />
      </PeopleBridgeHub>
    </PeopleProvider>
  );
}
