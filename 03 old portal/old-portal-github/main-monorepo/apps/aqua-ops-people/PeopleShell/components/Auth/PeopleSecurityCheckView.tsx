...
export const SecurityCheckView: React.FC = () => {
  const { handleVerifyMfa, currentUser } = usePeopleContext();
  ...
  return (
    <div className="min-h-screen bg-[var(--people-bg-color)] flex items-center justify-center p-6 font-sans relative overflow-hidden">
      {/* Background Ambience */}
      ...
      <motion.div
        ...
        className="max-w-md w-full glass-card p-10 rounded-[var(--radius-card-lg)] border border-[var(--people-widget-border)] relative z-10 text-center"
      >
        ...
        <h1 className="text-3xl font-bold text-[var(--people-widget-text)] mb-2 tracking-tight">Security Check</h1>
        <p className="text-[var(--people-widget-text-muted)] text-sm mb-10 leading-relaxed">
          We've sent a 6-digit verification code to <br />
          <span className="text-[var(--people-widget-text)] font-medium">{currentUser?.email}</span>
        </p>
        ...
        <div className="flex justify-between gap-2 mb-8">
          {pin.map((digit, i) => (
            <input
              ...
              className={`w-12 h-16 bg-[var(--people-widget-surface-color-1-glass)]/5 border rounded-xl text-center text-2xl font-bold text-[var(--people-widget-text)] transition-all focus:outline-none focus:ring-2 focus:ring-[var(--people-widget-primary-color-1)]/50 ${
                error ? 'border-[var(--people-widget-error)]/50 bg-[var(--people-widget-error)]/5' : 'border-[var(--people-widget-border)] focus:border-[var(--people-widget-primary-color-1)]/50'
              }`}
            />
          ))}
        </div>
        ...
        <div className="mt-8 flex items-center justify-between text-xs font-bold uppercase tracking-widest">
           <div className="text-[var(--people-widget-text-muted)] flex items-center gap-2">
           ...
           {timer === 0 ? (
             ...
             <span className="text-[var(--people-widget-text-muted)]/50">Resend Code</span>
           )}
        </div>

        <div className="mt-10 pt-8 border-t border-[var(--people-widget-border)] flex items-center justify-center gap-2 text-[var(--people-widget-text-muted)]/20">
