...
import { AppProvider } from '@CRMShell/bridge/CRMAppContext';
import { ModalProvider } from '@CRMShell/bridge/CRMModalContext';
import { useCRMLogic } from '@CRMShell/logic/CRMuseCRMLogic';
import { AppFrame } from '@CRMShell/AppFrame/CRMAppFrame';
import { CRMBridgeHub } from '@CRMShell/bridge/CRMBridgeHub';
    ...
    const unsub = onBridgeMessage((msg) => {
      ...
      if (msg.type === 'BRIDGE_THEME') {
        ...
        Object.entries(theme).forEach(([key, value]) => {
          root.style.setProperty(`--crm-widget-${key}`, value as string);
        });
      }
    });
    return unsub;
  ...
  useEffect(() => {
    if (ready) {
      ...
      if (typeof window !== 'undefined' && window.parent !== window) {
        sendBridgeMessage(window.parent, 'BRIDGE_READY', { viewId }, 'aqua-crm');
      }
    }
