...
} from '@CRMShell/bridge/data/CRMmockData';
import { loadSeedData } from '@CRMShell/bridge/data/CRMdb';
import { initialMasterConfig } from '@CRMShell/bridge/config/CRMmasterConfig';
import { BridgeProvisioning } from '@CRMShell/bridge/CRMprovisioning';
import { agencyConfig as defaultAgencyConfig } from '@CRMShell/bridge/config/CRMagencyConfig';
import { useSyncStore } from '@CRMShell/hooks/CRMuseSyncStore';
import { BridgeAPI } from '@CRMShell/bridge/CRMapi';

/**
 * useCoreLogic — The Central Data Kernel
 * 
 ...
 */
export function useCoreLogic(persisted: any) {
  ...
  const notifyHost = (key: string, value: any) => {
    if (typeof window !== 'undefined' && window.parent !== window) {
      import('@aqua/bridge/postMessage').then(m => {
        m.sendBridgeMessage(window.parent, 'BRIDGE_STATE_UPDATED', { key, value }, 'aqua-crm');
      });
    }
  };
  ...
  /**
   * Called by useAuthLogic immediately after a successful login,
  ...
  const addLog = (action: string, details: string, type: any = 'system', clientId?: string) => {
    const newLog: LogEntry = { 
      ...
      userId: 0, // Resolved at useCRMLogic level 
      userName: 'System', 
      action, 
      details, 
      type 
  ...
  return {
    ...
    handleImpersonate: (clientId: string) => {
      // This will be overridden in useCRMLogic with actual auth/shell logic, 
