...
export default function EmbedPage() {
    ...
    const unsub = onBridgeMessage((msg) => {
      if (msg.type === 'BRIDGE_AUTH') {
        ...
        const session = {
          ...
          productAccess: ['aqua-ops-revenue'],
      ...
      if (msg.type === 'BRIDGE_THEME') {
        ...
        Object.entries(theme).forEach(([key, value]) => {
          root.style.setProperty(`--revenue-widget-${key}`, value as string);
        });
      }
    });
    return unsub;
  ...
  useEffect(() => {
    if (ready) {
      ...
      if (typeof window !== 'undefined' && window.parent !== window) {
        sendBridgeMessage(window.parent, 'BRIDGE_READY', { viewId }, 'aqua-ops-revenue');
      }
    }
  ...
  return (
    <RevenueProvider value={{ ...logic }} >
      <RevenueBridgeHub>
        <RevenueFrame portalMode="user" />
      </RevenueBridgeHub>
    </RevenueProvider>
  );
}
