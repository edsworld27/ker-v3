...
import { sidebarItemUI as ui } from './Revenueui';
import { useTheme } from '@RevenueShell/hooks/RevenueuseTheme';
...
export const SidebarItem: React.FC<SidebarItemProps> = ({
  ...
  return (
    <button
      ...
      style={active ? {
        backgroundColor: 'color-mix(in srgb, var(--revenue-widget-primary-color-1) 10%, transparent)',
        color: 'var(--revenue-widget-primary-color-1)'
      } : {}}
    >
      ...
      {!collapsed && badge && !hasChildren && (
        <span className={`${ui.badge.base} ${active ? '' : ui.badge.inactive}`} style={active ? {
          backgroundColor: 'color-mix(in srgb, var(--revenue-widget-primary-color-1) 20%, transparent)'
        } : {}}>
          {badge}
        </span>
      )}
