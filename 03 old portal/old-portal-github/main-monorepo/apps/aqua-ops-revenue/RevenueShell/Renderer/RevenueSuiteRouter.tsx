...
export const SuiteRouter: React.FC<SuiteRouterProps> = ({ 
  ...
  const { portalView } = useRevenueContext();

  // Force re-render when RevenueRegistration updates
  const [, setTick] = useState(0);
  useEffect(() => {
    return RevenueRegistration.subscribe(() => setTick(t => t + 1));
  }, []);
  ...
  const Component = RevenueRegistration.resolve(componentIdToResolve) || SmartRegistry[componentIdToResolve];
  ...
  if (defaultViewId && componentIdToResolve !== defaultViewId) {
    const DefaultComponent = RevenueRegistration.resolve(defaultViewId) || SmartRegistry[defaultViewId];
    if (DefaultComponent) {
