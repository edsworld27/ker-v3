...
export const CustomPageView: React.FC<CustomPageViewProps> = ({ pageId }) => {
  const { customPages, portalView } = useAppContext();
  // Fallback: derive pageId from portalView if not provided as prop
  // (DynamicRenderer mounts views with empty props).
  const effectiveId = pageId ?? portalView ?? '';
  const page = customPages.find(p => p.id === effectiveId);
