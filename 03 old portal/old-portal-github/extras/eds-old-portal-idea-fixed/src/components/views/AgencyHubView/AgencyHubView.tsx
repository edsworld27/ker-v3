...
export const AgencyHubView: React.FC<AgencyHubViewProps> = (props) => {
  const ctx = useAppContext();
  const modalCtx = useModalContext();
  const currentUser = props.currentUser ?? ctx.currentUser ?? null;
  const users = props.users ?? ctx.users ?? [];
  const tickets = props.tickets ?? ctx.tickets ?? [];
  const aiSessions = props.aiSessions ?? [];
  const handleViewChange = props.handleViewChange ?? ctx.handleViewChange;
  const setShowEmployeeManagementModal = props.setShowEmployeeManagementModal ?? modalCtx.setShowEmployeeManagementModal;
