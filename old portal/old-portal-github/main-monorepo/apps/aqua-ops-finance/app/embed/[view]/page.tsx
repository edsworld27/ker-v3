...
export default function EmbedPage() {
    ...
    const unsub = onBridgeMessage((msg) => {
      if (msg.type === 'BRIDGE_AUTH') {
        ...
        const session = {
          ...
          productAccess: ['aqua-ops-finance'],
      ...
      if (msg.type === 'BRIDGE_THEME') {
        ...
        Object.entries(theme).forEach(([key, value]) => {
          root.style.setProperty(`--finance-widget-${key}`, value as string);
        });
      }
    });
    return unsub;
  ...
  useEffect(() => {
    if (ready) {
      ...
      if (typeof window !== 'undefined' && window.parent !== window) {
        sendBridgeMessage(window.parent, 'BRIDGE_READY', { viewId }, 'aqua-ops-finance');
      }
    }
  ...
  return (
    <FinanceProvider value={{ ...logic }} >
      <FinanceBridgeHub>
        <FinanceFrame portalMode="user" />
      </FinanceBridgeHub>
    </FinanceProvider>
  );
}
