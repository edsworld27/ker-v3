...
import { FinanceProvider } from '@FinanceShell/bridge/FinanceContext';
// Initialize Bridge
import { useFinanceLogic } from '@FinanceShell/logic/FinanceuseFinanceLogic';
import { FinanceFrame } from '@FinanceShell/AppFrame/FinanceAppFrame';
import { FinanceBridgeHub } from '@FinanceShell/bridge/FinanceBridgeHub';
 ...
 * 3. Provide the FinanceContext to all downstream components.
...
export default function App({ mode: initialMode = 'user', initialView }: AppProps) {
  ...
  useEffect(() => {
    Promise.resolve().then(() => setTemplatesReady(true))
      .catch(err => {
  ...
  useEffect(() => {
    ...
    if (agencyConfig?.identity) {
      root.style.setProperty('--finance-primary-color', agencyConfig.identity.primaryColor || '#4f46e5');
      root.style.setProperty('--finance-secondary-color', agencyConfig.identity.secondaryColor || '#10b981');
    }
  ...
  // the FinanceFrame will internally render the Auth views based on the 'step' state.
  
  return (
    <FinanceProvider value={appContextValue as any}>
      <FinanceBridgeHub>
        <FinanceFrame portalMode={portalMode} />
      </FinanceBridgeHub>
    </FinanceProvider>
  );
}
