...
import { DynamicViewRenderer } from '../Renderer/FinanceDynamicViewRenderer';
import { ModalManager } from '@FinanceShell/components/FinanceModalManager';
import { SmartRegistry } from '@FinanceShell/components/FinanceSmartRegistry';
...
import { WelcomeScreen } from '../components/Auth/FinanceWelcomeScreen';
import { LoginView } from '../components/Auth/FinanceLoginView';
import { SecurityCheckView } from '../components/Auth/FinanceSecurityCheckView';
    ...
    if (this.state.error) {
      ...
      return (
        <div className="flex flex-col items-center justify-center p-16 text-center">
          <p className="text-xl font-bold text-[var(--finance-widget-error)] mb-2">Something went wrong in this view</p>
          <p className="text-sm text-[var(--finance-text-color-muted)] mb-4 font-mono">{err.message}</p>
...
interface FinanceFrameProps {
  portalMode: 'demo' | 'user';
}

/**
 * FinanceFrame — The Structural Host
 * 
 ...
 */
export const FinanceFrame: React.FC<FinanceFrameProps> = ({ portalMode }) => {
  // Track FinanceRegistration updates so ViewErrorBoundary auto-resets on template load
  const [registryTick, setRegistryTick] = useState(0);
  useEffect(() => FinanceRegistration.subscribe(() => setRegistryTick(t => t + 1)), []);
            ...
            <main className="flex-1 overflow-hidden relative bg-black/40 flex flex-col">
              ...
              {impersonatedUserEmail && (
                <div className="shrink-0 h-10 backdrop-blur-md flex items-center justify-between px-8 text-white z-50"
                     style={{ backgroundColor: 'color-mix(in srgb, var(--finance-primary-color) 90%, transparent)' }}>
