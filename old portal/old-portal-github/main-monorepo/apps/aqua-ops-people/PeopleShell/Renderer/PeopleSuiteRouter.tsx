...
export const SuiteRouter: React.FC<SuiteRouterProps> = ({ 
  ...
  const { portalView } = usePeopleContext();

  // Force re-render when PeopleRegistration updates
  const [, setTick] = useState(0);
  useEffect(() => {
    return PeopleRegistration.subscribe(() => setTick(t => t + 1));
  }, []);
  ...
  const Component = PeopleRegistration.resolve(componentIdToResolve) || SmartRegistry[componentIdToResolve];
  ...
  if (defaultViewId && componentIdToResolve !== defaultViewId) {
    const DefaultComponent = PeopleRegistration.resolve(defaultViewId) || SmartRegistry[defaultViewId];
    if (DefaultComponent) {
