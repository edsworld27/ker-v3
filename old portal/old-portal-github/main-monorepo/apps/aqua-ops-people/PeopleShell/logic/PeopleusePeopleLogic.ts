...
import { usePeopleCore } from '@PeopleShell/logic/PeopleusePeopleCore';
import { usePeopleAuth } from '@PeopleShell/logic/PeopleuseAuthLogic';
import { usePeopleShellLogic } from '@PeopleShell/logic/PeopleuseShellLogic';
...
/**
 * usePeopleLogic (The Facade Orchestrator)
 * 
...
export function usePeopleLogic() {
  ...
  const core = usePeopleCore(persisted);
  const {
  ...
  const auth = usePeopleAuth(persisted, {
    users,
    currentUserEmail,
    addLog,
    sendNotification,
  ...
  const shell = usePeopleShellLogic(persisted, { addLog });
  ...
  // ── Demo Mode Engine ──────────────────────────────────────────────────────
  const setDemoState = () => {
    import('@PeopleShell/bridge/demo/PeopledemoState').then(module => {
  ...
  // Keep ref in sync so usePeopleAuth can call it
