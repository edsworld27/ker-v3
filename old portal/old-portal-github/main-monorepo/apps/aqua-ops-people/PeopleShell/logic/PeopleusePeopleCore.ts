...
} from '@PeopleShell/bridge/data/PeoplemockData';
import { loadSeedData } from '@PeopleShell/bridge/data/Peopledb';
import { initialMasterConfig } from '@PeopleShell/bridge/config/PeoplemasterConfig';
import { BridgeProvisioning } from '@PeopleShell/bridge/Peopleprovisioning';
import { agencyConfig as defaultAgencyConfig } from '@PeopleShell/bridge/config/PeopleagencyConfig';
import { useSyncStore } from '@PeopleShell/hooks/PeopleuseSyncStore';
import { BridgeAPI } from '@PeopleShell/bridge/Peopleapi';

/**
 * usePeopleCore — The Central Data Kernel
 * 
 ...
 */
export function usePeopleCore(persisted: any) {
  ...
  /**
   * Called by usePeopleAuth immediately after a successful login,
  ...
  const addLog = (action: string, details: string, type: any = 'system', clientId?: string) => {
    const newLog: LogEntry = { 
      ...
      userId: 0, // Resolved at usePeopleLogic level 
      userName: 'System', 
      action, 
      details, 
      type 
  ...
  return {
    ...
    handleImpersonate: (clientId: string) => {
      // This will be overridden in usePeopleLogic with actual auth/shell logic, 
