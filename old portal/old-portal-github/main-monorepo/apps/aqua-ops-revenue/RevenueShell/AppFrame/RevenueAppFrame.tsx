...
import { RevenueSidebarContent } from '../Sidebar/RevenueSidebarContent';
import { DynamicViewRenderer } from '../Renderer/RevenueDynamicViewRenderer';
import { ModalManager } from '@RevenueShell/components/RevenueModalManager';
import { SmartRegistry } from '@RevenueShell/components/RevenueSmartRegistry';
...
import { WelcomeScreen } from '../components/Auth/RevenueWelcomeScreen';
import { LoginView } from '../components/Auth/RevenueLoginView';
import { SecurityCheckView } from '../components/Auth/RevenueSecurityCheckView';
    ...
    if (this.state.error) {
      ...
      return (
        <div className="flex flex-col items-center justify-center p-16 text-center">
          <p className="text-xl font-bold text-[var(--revenue-widget-error)] mb-2">Something went wrong in this view</p>
          <p className="text-sm text-[var(--revenue-widget-text-muted)] mb-4 font-mono">{err.message}</p>
...
interface RevenueFrameProps {
  portalMode: 'demo' | 'user';
}

/**
 * RevenueFrame — The Structural Host
 * 
 ...
 */
export const RevenueFrame: React.FC<RevenueFrameProps> = ({ portalMode }) => {
  // Track RevenueRegistration updates so ViewErrorBoundary auto-resets on template load
  const [registryTick, setRegistryTick] = useState(0);
  useEffect(() => RevenueRegistration.subscribe(() => setRegistryTick(t => t + 1)), []);
