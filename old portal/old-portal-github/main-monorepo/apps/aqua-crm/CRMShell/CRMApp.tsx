...
import { BridgeHub } from '@CRMShell/bridge/CRMBridgeHub';
...
export default function App({ mode: initialMode = 'user', initialView }: AppProps) {
  ...
  useEffect(() => {
    import('@CRMShell/CRMTemplates/CRMindex')
      .then(m => m.registerCrmApp())
  ...
  useEffect(() => {
    ...
    if (agencyConfig?.identity) {
      ...
      root.style.setProperty('--crm-widget-secondary', agencyConfig.identity.secondaryColor || '#10b981');
    }
