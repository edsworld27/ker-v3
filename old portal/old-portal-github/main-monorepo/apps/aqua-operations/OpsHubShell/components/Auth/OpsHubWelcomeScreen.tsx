...
import { Step } from '@OpsHubShell/bridge/types';
...
export function WelcomeScreen() {
  ...
  const OptionCard = ({ icon: Icon, title, description, actionLabel, onClick, color }) => (
    <button
      onClick={onClick}
      className="group glass-card rounded-[var(--radius-button)] p-6 border border-[var(--opshub-widget-border)] text-left hover:border-[var(--people-widget-primary-color-1)]/50 bg-[var(--opshub-widget-surface-1-glass)]/5 transition-all duration-200 hover:scale-[1.02] flex flex-col"
    >
      <div className="w-10 h-10 rounded-[var(--radius-button)] flex items-center justify-center mb-4 border border-[var(--opshub-widget-border)]"
           style={{ backgroundColor: color || 'var(--opshub-widget-surface-1)' }}>
        <Icon className="w-5 h-5 text-white" />
      </div>
      <h3 className="font-bold text-[var(--opshub-widget-text)] mb-1">{title}</h3>
      <p className="text-[var(--opshub-widget-text-muted)] text-sm leading-relaxed flex-grow">
        {description}
      </p>
  ...
  return (
    <div className="flex items-center justify-center w-full min-h-screen relative z-10 px-6 bg-[var(--opshub-widget-bg-color-1)]" data-design-static="true">
      <div className="absolute inset-0" style={{ background: `radial-gradient(circle at 50% 30%, ${primaryColor}22 0%, var(--opshub-widget-bg-color-1) 60%)` }} />
      ...
      >
        <div className="w-16 h-16 rounded-[var(--radius-button)] flex items-center justify-center mx-auto mb-8 shadow-xl border border-[var(--opshub-widget-border)]"
             style={{ backgroundColor: primaryColor, boxShadow: `0 20px 40px ${primaryColor}40` }}>
          <Zap className="w-9 h-9 text-white" />
        </div>

        <h1 className="text-4xl font-bold mb-3 tracking-tight text-[var(--opshub-widget-text)]">Welcome to Aqua Portal</h1>
        <p className="text-[var(--opshub-widget-text-muted)] mb-12 text-lg">
        ...
        <p className="text-[10px] text-[var(--opshub-widget-text-muted)] mt-10 uppercase tracking-widest">
