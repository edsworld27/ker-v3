...
import { OpsHubSidebarContent } from '../Sidebar/OpsHubSidebarContent';
import { DynamicViewRenderer } from '../Renderer/OpsHubDynamicViewRenderer';
import { ModalManager } from '@OpsHubShell/components/OpsHubModalManager';
import { SmartRegistry } from '@OpsHubShell/components/OpsHubSmartRegistry';
import { BridgeRegistry } from '@OpsHubShell/bridge/OpsHubRegistration';

// Core Auth Views (Hardcoded in Shell for reliability)
import { WelcomeScreen } from '../components/Auth/OpsHubWelcomeScreen';
import { LoginView } from '../components/Auth/OpsHubLoginView';
import { SecurityCheckView } from '../components/Auth/OpsHubSecurityCheckView';
    ...
    if (this.state.error) {
      ...
      return (
        <div className="flex flex-col items-center justify-center p-16 text-center">
          <p className="text-xl font-bold text-[var(--opshub-widget-error)] mb-2">Something went wrong in this view</p>
          <p className="text-sm text-[var(--opshub-widget-text-muted)] mb-4 font-mono">{err.message}</p>
 ...
 */
export const AppFrame: React.FC<AppFrameProps> = ({ portalMode }) => {
  // Track BridgeRegistry updates so ViewErrorBoundary auto-resets on template load
  const [registryTick, setRegistryTick] = useState(0);
  useEffect(() => BridgeRegistry.subscribe(() => setRegistryTick(t => t + 1)), []);
