...
export const SuiteRouter: React.FC<SuiteRouterProps> = ({ 
  ...
  const { portalView } = useAppContext();
  ...
  const Component = BridgeRegistry.resolve(componentIdToResolve) || SmartRegistry[componentIdToResolve];
  ...
  if (defaultViewId && componentIdToResolve !== defaultViewId) {
    const DefaultComponent = BridgeRegistry.resolve(defaultViewId) || SmartRegistry[defaultViewId];
    if (DefaultComponent) {
