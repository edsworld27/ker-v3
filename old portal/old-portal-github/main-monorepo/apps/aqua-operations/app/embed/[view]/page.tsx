...
import { OpsHubBridgeHub } from '@OpsHubShell/bridge/OpsHubBridgeHub';
    ...
    const unsub = onBridgeMessage((msg) => {
      ...
      if (msg.type === 'BRIDGE_THEME') {
        ...
        Object.entries(theme).forEach(([key, value]) => {
          root.style.setProperty(`--opshub-widget-${key}`, value as string);
        });
      }
    });
    return unsub;
  ...
  useEffect(() => {
    if (ready) {
      ...
      if (typeof window !== 'undefined' && window.parent !== window) {
        sendBridgeMessage(window.parent, 'BRIDGE_READY', { viewId }, 'aqua-operations');
      }
    }
