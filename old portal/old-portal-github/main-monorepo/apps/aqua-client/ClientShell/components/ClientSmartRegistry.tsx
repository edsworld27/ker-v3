...
import { autoComponentMap } from './ClientautoComponentMap';
import { ClientRegistration } from '@ClientShell/bridge/ClientRegistration';
...
export const SmartRegistry = new Proxy(autoComponentMap, {
  get(target, prop: string | symbol) {
    ...
    const bridgeComponent = ClientRegistration.resolve(prop);
    ...
    const bridgeFallback = ClientRegistration.resolve(normalizedProp);
    ...
    console.warn(`[ClientSmartRegistry] Missing Component: ${String(prop)}`);
    return (props: any) => (
      <div className="p-4 border border-dashed border-red-500/50 bg-red-500/10 rounded-md text-red-500 flex flex-col items-center justify-center min-h-[100px] w-full text-center">
        <code className="text-[10px] font-mono">Client: {String(prop)}</code>
      </div>
    );
  }
});
