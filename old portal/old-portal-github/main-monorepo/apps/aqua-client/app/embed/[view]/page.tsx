...
import { AppProvider } from '@ClientShell/bridge/ClientAppContext';
import { ModalProvider } from '@ClientShell/bridge/ClientModalContext';
import { useClientLogic } from '@ClientShell/logic/ClientuseClientLogic';
import { AppFrame } from '@ClientShell/AppFrame/ClientAppFrame';
import { ClientBridgeHub } from '@ClientShell/bridge/ClientBridgeHub';
...
export default function EmbedPage() {
    ...
    const unsub = onBridgeMessage((msg) => {
      ...
      if (msg.type === 'BRIDGE_THEME') {
        ...
        Object.entries(theme).forEach(([key, value]) => {
          root.style.setProperty(`--client-widget-${key}`, value as string);
        });
      }
    });
    return unsub;
  ...
  useEffect(() => {
    if (ready) {
      ...
      if (typeof window !== 'undefined' && window.parent !== window) {
        sendBridgeMessage(window.parent, 'BRIDGE_READY', { viewId }, 'aqua-client');
      }
    }
  ...
  return (
    <AppProvider value={{ ...logic }}>
      <ModalProvider>
        <ClientBridgeHub>
          <AppFrame portalMode="user" />
        </ClientBridgeHub>
      </ModalProvider>
    </AppProvider>
  );
