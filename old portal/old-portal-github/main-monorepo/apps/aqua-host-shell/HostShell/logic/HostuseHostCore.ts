...
import {
  HostUser, Client, LogEntry, HostNotification, Step, HostPortalTier, Agency, ClientStage, BridgeSession
} from '@HostShell/bridge/types';
import {
...
} from '@HostShell/bridge/data/HostmockData';
import { loadSeedData } from '@HostShell/bridge/data/Hostdb';
import { initialMasterConfig } from '@HostShell/bridge/config/HostmasterConfig';
import { BridgeProvisioning } from '@HostShell/bridge/Hostprovisioning';
import { agencyConfig as defaultAgencyConfig } from '@HostShell/bridge/config/HostagencyConfig';
import { useSyncStore } from '@HostShell/hooks/HostuseSyncStore';
import { BridgeAPI } from '@HostShell/bridge/Hostapi';

/**
 * useHostCore — The Central Data Kernel
 * 
 ...
 * specialized hooks in the Templates directory to maintain a lean HostApp Shell.
 */
export function useHostCore(persisted: any) {
  const [step, setStep] = useState<Step>(persisted?.setupComplete ? 'login' : 'welcome');
  const [appMode, setAppMode] = useState<any>('operations');
  const [portalTier, setHostPortalTier] = useState<HostPortalTier>(persisted?.portalTier ?? 'agency');
  ...
  const [users, setUsersInternal] = useState<HostUser[]>(persisted?.users ?? initialUsers);
  ...
  /**
   * Called by useHostAuth immediately after a successful login,
  ...
  const addLog = (action: string, details: string, type: any = 'system', clientId?: string) => {
    const newLog: LogEntry = { 
      ...
      userId: 0, // Resolved at useHostLogic level 
      userName: 'System', 
      action, 
      details, 
      type 
  ...
  return {
    ...
    handleImpersonate: (clientId: string) => {
      // This will be overridden in useHostLogic with actual auth/shell logic, 
    ...
    currentAgency: bridgeSession?.agency ?? { id: activeAgencyId, name: 'HostAqua Agency', isConfigured: true },
    adminStats: { 
