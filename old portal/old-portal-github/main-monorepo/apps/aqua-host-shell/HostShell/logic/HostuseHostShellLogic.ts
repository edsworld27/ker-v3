...
import { HostPortalView } from '@HostShell/bridge/types';
 ...
 */
export function useHostShellLogic(persistedState: any, deps: { addLog: any }) {
  const { addLog } = deps;
  ...
  const [activeHostPortalTheme, setActiveHostPortalTheme] = useState(persistedState?.theme ?? 'dark');
  ...
  const [portalView, setHostPortalView] = useState<HostPortalView | string>(persistedState?.portalView ?? 'app-client');
  ...
  }, [sidebarCollapsed, activeHostPortalTheme, sidebarStack, portalView, appLogo]);
  ...
  return {
    ...
    dropdownRef,
    activeHostPortalTheme, setActiveHostPortalTheme,
