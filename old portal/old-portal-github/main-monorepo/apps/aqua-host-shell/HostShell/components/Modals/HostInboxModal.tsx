...
export function InboxModal({ onClose }: Props) {
  const { agencyMessages, currentUser } = useHostContext();
    ...
    <div className="bg-[#0f172a] border border-white/10 rounded-2xl shadow-2xl overflow-hidden max-h-[70vh] flex flex-col">
      <div className="flex items-center justify-between px-6 py-4 border-b border-white/10">
        <div className="flex items-center gap-2">
          <MessageSquare className="w-5 h-5 text-[var(--host-widget-primary-color-1)]" />
          <h2 className="text-lg font-semibold text-white">Inbox</h2>
          {messages.length > 0 && (
            <span className="px-2 py-0.5 text-[10px] font-bold bg-[var(--host-widget-primary-color-1)]/20 text-[var(--host-widget-primary-color-1)] rounded-full">
              {messages.length}
            </span>
          )}
        </div>
      ...
      <div className="overflow-y-auto flex-1 divide-y divide-white/5">
        ...
        {messages.map((msg: any, i: number) => (
          <div key={msg.id || i} className="flex items-start gap-3 px-6 py-4 hover:bg-white/5 transition-colors">
            <div className="w-8 h-8 rounded-full bg-[var(--host-widget-primary-color-1)]/20 flex items-center justify-center shrink-0">
              <User className="w-4 h-4 text-[var(--host-widget-primary-color-1)]" />
            </div>
