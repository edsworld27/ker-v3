    ...
    <div className="rounded-2xl border border-white/8 bg-brand-black-card overflow-hidden">
      <div className="flex items-center gap-2 px-5 py-3 border-b border-white/5 bg-brand-black-soft/40">
        <input
          ...
          className="flex-1 bg-transparent text-sm font-display font-semibold text-brand-cream focus:outline-none"
        />
        <div className="flex items-center gap-1 text-[11px]">
          <button disabled={!canUp} onClick={() => moveGroup(group.id, -1)} className="px-2 py-0.5 text-brand-cream/40 hover:text-brand-cream disabled:opacity-25">↑</button>
          <button disabled={!canDown} onClick={() => moveGroup(group.id, 1)} className="px-2 py-0.5 text-brand-cream/40 hover:text-brand-cream disabled:opacity-25">↓</button>
          <button onClick={() => { if (confirm(`Delete "${group.heading}" and ${group.items.length} item${group.items.length === 1 ? "" : "s"}?`)) deleteGroup(group.id); }} className="px-2 py-0.5 text-brand-cream/40 hover:text-brand-orange">Delete</button>
        </div>
      </div>
    ...
    <div className="rounded-lg border border-white/5 bg-brand-black p-3 space-y-2">
      <div className="flex items-center gap-2">
        <input
          ...
          className="flex-1 bg-transparent text-sm font-medium text-brand-cream focus:outline-none"
        />
        <div className="flex items-center gap-1 text-[11px]">
          <button disabled={!canUp} onClick={() => moveItem(groupId, item.id, -1)} className="px-1.5 text-brand-cream/40 hover:text-brand-cream disabled:opacity-25">↑</button>
          <button disabled={!canDown} onClick={() => moveItem(groupId, item.id, 1)} className="px-1.5 text-brand-cream/40 hover:text-brand-cream disabled:opacity-25">↓</button>
          <button onClick={() => deleteItem(groupId, item.id)} className="px-1.5 text-brand-cream/40 hover:text-brand-orange">×</button>
        </div>
      </div>
      <textarea
        value={a}
        ...
        placeholder="Answer"
        rows={3}
        className="w-full bg-transparent text-xs text-brand-cream/70 focus:outline-none resize-y leading-relaxed"
      />
    </div>
  );
}
